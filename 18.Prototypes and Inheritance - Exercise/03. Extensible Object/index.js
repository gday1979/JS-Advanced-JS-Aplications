function extensibleObject() {
  ...
  instance.extend = function (template) {
    // Създаваме си метод extend на инстанцията, който приема друг обект(template)
    Object.entries(template).forEach(([key, value]) => {
      // Взимаме всички, KVP на обекта template и минаваме по тях
      if (typeof value === "function") {
        // Ако value e функция(метод)
        prototype[key] = value; // Я добавяме към прототипа
      } else {
        // Ако value не е функция(пропърти)
